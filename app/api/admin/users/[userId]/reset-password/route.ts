...
import { getAdminPasswordResetEmailHtml } from '@/lib/email-templates';
...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      const html = getAdminPasswordResetEmailHtml({
        userName,
        temporaryPassword,
      });
      ...
      await sendEmail({
        to: targetUser.email,
        subject: 'Your EaseMail Password Has Been Reset',
        html,
      });
      ...
      console.error('Failed to send password reset email to', targetUser.email, emailError);
      // Password was changed but email failed - still return success but warn
      return NextResponse.json({
        success: true,
    ...
    return ApiErrors.internalError('Failed to reset password');
  }
}
